function [y, t, info, ind_over, ind_under, frames] = wfm3read(filename, datapoints, step, startind)

% UPDATE 2026 VBP:
% Modified version of wfm2read.m
%
% This copy was updated to avoid errors when reading NULL-terminated
% character fields from Tektronix .wfm files.
%
% The original code used statements like:
%     char(dummy(1:find(dummy==0)))
%
% In some files, find(dummy==0) can return multiple indices or no index,
% which causes MATLAB to throw:
%     "Colon operands must be real scalars."
%
% To fix this, those lines were replaced with a safer NULL-string reader
% that uses only the first NULL character:
%     nullIndex = find(dummy==0, 1, 'first');
%
% If a NULL character is found, the text before the first NULL is used.
% If no NULL character is found, the full character field is used.
%
% This update does not change how the waveform data are read. It only makes
% the metadata/label/unit string parsing more robust.

% Helper function for reading NULL-terminated strings from fixed-width
% character fields in the .wfm header.
%
% Some Tektronix .wfm metadata fields are stored as fixed-length character
% arrays padded with NULL values. The original code assumed there would be
% exactly one NULL character, but MATLAB errors if the index is empty or if
% multiple indices are returned. This function safely uses the first NULL
% character when present.
% function out = readNullString(dummy)
%     nullIndex = find(dummy==0, 1, 'first');
% 
%     if isempty(nullIndex)
%         out = char(dummy(:).');
%     else
%         out = char(dummy(1:nullIndex-1).');
%     end
% end
...
if isempty(nullIndex)
    info.waveform_label = char(dummy(:).');
else
    info.waveform_label = char(dummy(1:nullIndex-1).');
end
...
if isempty(nullIndex)
    info.ed1.units = char(dummy(:).');
else
    info.ed1.units = char(dummy(1:nullIndex-1).');
end
...
if isempty(nullIndex)
    info.ed2.units = char(dummy(:).');
else
    info.ed2.units = char(dummy(1:nullIndex-1).');
end
...
values=double(fread(fid,nop,format,info.num_bytes_per_point*(step-1),byteorder));% %read data values from curve buffer
...
%     warning('WFM2read:OverRangeValues',[int2str(length(ind_over)), ' over range value(s) in file ' filename]); 
% end
% if length(ind_under) %#ok
%     warning('WFM2read:UnderRangeValues',[int2str(length(ind_under)), ' under range value(s) in file ' filename]); 
% end
...
%             warning('WFM2read:OverRangeValues',[int2str(length(frames.(['frame' num2str(frm)]).ind_over)), ' over range value(s) in file ' filename ', frame ' num2str(frm)]); 
%         end
%         if length(frames.(['frame' num2str(frm)]).ind_under) %#ok
%             warning('WFM2read:UnderRangeValues',[int2str(length(frames.(['frame' num2str(frm)]).ind_under)), ' under range value(s) in file ' filename ', frame ' num2str(frm)]); 
%         end
    end
end
fclose(fid);
